...
Peripheral.prototype.connectCancel = function(callback) {
  this.once('connect', function(err) {
    if (callback) { callback(null); }
  });

  this._noble.connectCancel(this.id);
};
...
Peripheral.prototype.updateLinkParam = function(interval, latency, timeout, callback) {
  if (callback) {
    this.once('connUpdateComplete', function() {
      callback(null);
    });
  }

  this._noble.updateConnParameters(this.id, interval, latency, timeout);
};
...
Peripheral.prototype.encrypt = function(setting, callback) {
  if (callback) {
    this.once('encrypt', function(encrypt) {
      callback(null, encrypt);
    });
  }
  
  this._noble.encrypt(this.id, setting);
};
