...
NobleBindings.prototype.encrypt = function(peripheralUuid, setting) {
  this._aclStreams[this._handles[peripheralUuid]].encryptWithSetting(setting);
};
...
NobleBindings.prototype.onEncryptChange = function(handle, encrypt) {
  var uuid = this._handles[handle],
      aclStream = this._aclStreams[handle];

  if (aclStream) {
    aclStream.pushEncrypt(encrypt);
  }

  this.emit('encrypt', uuid, encrypt);
};
...
NobleBindings.prototype.readDesc = function(peripheralUuid, serviceUuid, characteristicUuid) {
  var handle = this._handles[peripheralUuid];
  var gatt = this._gatts[handle];

  if (gatt) {
    gatt.readDesc(serviceUuid, characteristicUuid);
  } else {
    console.warn('noble warning: unknown peripheral ' + peripheralUuid);
  }
};

NobleBindings.prototype.onReadDesc = function(address, serviceUuid, characteristicUuid, data) {
  var uuid = address.split(':').join('').toLowerCase();

  this.emit('readDesc', uuid, serviceUuid, characteristicUuid, data);
};
