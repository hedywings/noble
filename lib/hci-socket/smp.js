...
Smp.prototype.sendPairingRequestWithSetting = function(setting) {
  var authReq = 0x00,
      ioCap = setting.ioCap ? setting.ioCap : 0x03;

  if (setting.bond) { authReq = authReq | 0x01; }
  if (setting.mitm) { authReq = authReq | 0x02; }

  this._preq = new Buffer([
    SMP_PAIRING_REQUEST,
    ioCap, // IO capability
    0x00, // OOB data: Authentication data not present
    authReq, // Authentication requirement
    0x10, // Max encryption key size
    0x00, // Initiator key distribution: <none>
    0x01  // Responder key distribution: EncKey
  ]);
