...
Hci.prototype.reset = function() {
  ...
  debug('reset host - writing: ' + cmd.toString('hex'));
  this._socket.write(cmd);
};
...
Hci.prototype.setScanParameters = function(interval, window) {
  ...
  cmd.writeUInt16LE(interval, 5); // internal, ms * 1.6
...
Hci.prototype.createLeConn = function(address, addressType) {
  ...
  cmd.writeUInt16LE(linkParams.minInterval, 17); // min interval
  cmd.writeUInt16LE(linkParams.maxInterval, 19); // max interval
  cmd.writeUInt16LE(linkParams.latency, 21); // latency
  cmd.writeUInt16LE(linkParams.timeout, 23); // supervision timeout
...
Hci.prototype.createLeConnCancle = function(address, addressType) {
  var cmd = new Buffer(4);
...
Hci.prototype.updateConnParameters = function(handle, interval, latency, timeout) {
  ...
  cmd.writeUInt16LE(interval, 6); // min interval
  cmd.writeUInt16LE(interval, 8); // max interval
  cmd.writeUInt16LE(latency, 10); // latency
  cmd.writeUInt16LE(timeout, 12); // supervision timeout
  cmd.writeUInt16LE(0x0004, 14); // min ce length
  cmd.writeUInt16LE(0x0006, 16); // max ce length

  debug('update le conn params - writing: ' + cmd.toString('hex'));
  this._socket.write(cmd);
};
