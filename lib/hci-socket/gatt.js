  ...
  if (this._currentCommand && data.toString('hex') === this._currentCommand.buffer.toString('hex')) {
    ...
    for (var serviceUuid in this._services) {
      for (var characteristicUuid in this._characteristics[serviceUuid]) {
        var charLocation = this._characteristics[serviceUuid][characteristicUuid];
        if (Array.isArray(charLocation)) {
          charLocation.forEach(function (char) {
            if (char.valueHandle === valueHandle)
              self.emit('notification', self._address, serviceUuid, characteristicUuid, char.valueHandle, valueData);
          });
        } else {
          if (charLocation.valueHandle === valueHandle) {
            this.emit('notification', this._address, serviceUuid, characteristicUuid, charLocation.valueHandle, valueData);
          }
        }
      }
    }
    ...
    if (opcode !== ATT_OP_READ_BY_TYPE_RESP || characteristics[characteristics.length - 1].valueHandle === service.endHandle) {
      ...
      for (i = 0; i < characteristics.length; i++) {
        ...
        var characteristic = {
          ...
          handle: characteristics[i].valueHandle
        };
        ...
        var charLocation = this._characteristics[serviceUuid][characteristics[i].uuid];
        if (charLocation) {
          if (!Array.isArray(charLocation)) {
            var characteristicList = [];
            characteristicList.push(charLocation);
            this._characteristics[serviceUuid][characteristics[i].uuid] = characteristicList;
          }
...
Gatt.prototype.read = function(serviceUuid, characteristicUuid, characteristicHandle) {
  var characteristic = this.findChar(serviceUuid, characteristicUuid, characteristicHandle);
  ...
  var callback = function(data) {
    ...
    if (opcode === ATT_OP_READ_RESP || opcode === ATT_OP_READ_BLOB_RESP) {
      ...
      if (data.length === this._mtu) {
        ...
        this.emit('read', this._address, serviceUuid, characteristicUuid, characteristicHandle, readData);
      }
    } else {
      this.emit('read', this._address, serviceUuid, characteristicUuid, characteristicHandle, readData);
    }
...
Gatt.prototype.readDesc = function(serviceUuid, characteristicUuid, characteristicHandle) {
  var characteristic = this.findChar(serviceUuid, characteristicUuid, characteristicHandle);
  ...
  var callback = function(data) {
    ...
    if (opcode === ATT_OP_READ_RESP || opcode === ATT_OP_READ_BLOB_RESP) {
      ...
      if (data.length === this._mtu) {
        this._queueCommand(this.readBlobRequest(characteristic.endHandle, readData.length), callback);
      } else {
        this.emit('readDesc', this._address, serviceUuid, characteristicUuid, characteristicHandle, readData);
      }
    } else {      
      this.emit('readDesc', this._address, serviceUuid, characteristicUuid, characteristicHandle, readData);
    }
  ...
  this._queueCommand(this.readRequest(characteristic.endHandle), callback);
};

Gatt.prototype.write = function(serviceUuid, characteristicUuid, characteristicHandle, data, withoutResponse) {
  var characteristic = this.findChar(serviceUuid, characteristicUuid, characteristicHandle);

  if (withoutResponse) {
    this._queueCommand(this.writeRequest(characteristic.valueHandle, data, true), null, function() {
      this.emit('write', this._address, serviceUuid, characteristicUuid, characteristicHandle);
    }.bind(this));
  } else {
...
Gatt.prototype.broadcast = function(serviceUuid, characteristicUuid, characteristicHandle, broadcast) {
  var characteristic = this.findChar(serviceUuid, characteristicUuid, characteristicHandle);
  ...
  this._queueCommand(this.readByTypeRequest(characteristic.startHandle, characteristic.endHandle, GATT_SERVER_CHARAC_CFG_UUID), function(data) {
    ...
    if (opcode === ATT_OP_READ_BY_TYPE_RESP) {
      ...
      this._queueCommand(this.writeRequest(handle, valueBuffer, false), function(data) {
        ...
        if (opcode === ATT_OP_WRITE_RESP) {
          this.emit('broadcast', this._address, serviceUuid, characteristicUuid, characteristicHandle, broadcast);
        }
      }.bind(this));
    }
  }.bind(this));
...
Gatt.prototype.notify = function(serviceUuid, characteristicUuid, characteristicHandle, notify) {
  var characteristic = this.findChar(serviceUuid, characteristicUuid, characteristicHandle);
  ...
  this._queueCommand(this.readByTypeRequest(characteristic.startHandle, characteristic.endHandle, GATT_CLIENT_CHARAC_CFG_UUID), function(data) {
    ...
    if (opcode === ATT_OP_READ_BY_TYPE_RESP) {
      ...
      this._queueCommand(this.writeRequest(handle, valueBuffer, false), function(data) {
        ...
        if (opcode === ATT_OP_WRITE_RESP) {
          this.emit('notify', this._address, serviceUuid, characteristicUuid, characteristicHandle,  notify);
        }
      }.bind(this));
    }
  }.bind(this));
...
Gatt.prototype.discoverDescriptors = function(serviceUuid, characteristicUuid, characteristicHandle) {
  var characteristic = this.findChar(serviceUuid, characteristicUuid, characteristicHandle);
  var descriptors = [];
  ...
  var callback = function(data) {
    ...
    if (opcode !== ATT_OP_FIND_INFO_RESP || descriptors[descriptors.length - 1].handle === characteristic.endHandle) {
      ...
      this.emit('descriptorsDiscover', this._address, serviceUuid, characteristicUuid, characteristicHandle, descriptorUuids);
    } else {
...
Gatt.prototype.findChar = function(serviceUuid, characteristicUuid, characteristicHandle) {
  ...
  if (Array.isArray(characteristic))
    characteristic.forEach(function (char) {
      if (char.valueHandle === characteristicHandle)
        characteristic = char;
    });
