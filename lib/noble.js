...
function Noble() {
  ...
  this._bindings.on('encrypt', this.onEncrypt.bind(this));
...
Noble.prototype.readDesc = function(peripheralUuid, serviceUuid, characteristicUuid) {
   this._bindings.readDesc(peripheralUuid, serviceUuid, characteristicUuid);
};

Noble.prototype.onReadDesc = function(peripheralUuid, serviceUuid, characteristicUuid, data) {
  var characteristic = this._characteristics[peripheralUuid][serviceUuid][characteristicUuid];
...
Noble.prototype.encrypt = function(peripheralUuid, setting) {
  this._bindings.encrypt(peripheralUuid, setting);
};

Noble.prototype.onEncrypt = function(peripheralUuid, encrypt) {
  var peripheral = this._peripherals[peripheralUuid];

  if (peripheral) {
    peripheral.emit('encrypt', encrypt);
  } else {
    this.emit('warning', 'unknown peripheral ' + peripheralUuid + ' encrypt!');
  }
};
